...
static inline void
gen_svm_check_intercept_param(DisasContext *s, target_ulong pc_start, uint32_t type, uint64_t param)
{
...
crc32:
        ...
        case 0x13a:
            ...
            if (sse_op2 == SSE_SPECIAL) {
                ...
                case 0x21: /* insertps */
                    if (mod == 3) {
                        tcg_gen_ld_i32(cpu_tmp2_i32, cpu_env, offsetof(CPUState, xmm_regs[rm]
                                                                       .XMM_L((val >> 6) & 3)));
                    } else {
                        tcg_gen_qemu_ld32u(cpu_tmp0, cpu_A0, (s->base.mem_idx >> 2) - 1);
                        tcg_gen_trunc_tl_i32(cpu_tmp2_i32, cpu_tmp0);
                    }
                    tcg_gen_st_i32(cpu_tmp2_i32, cpu_env, offsetof(CPUState, xmm_regs[reg]
                                                                   .XMM_L((val >> 4) & 3)));
                    if ((val >> 0) & 1) {
                        tcg_gen_st_i32(tcg_const_i32(0 /*float32_zero*/), cpu_env, offsetof(CPUState, xmm_regs[reg].XMM_L(0)));
                    }
                    if ((val >> 1) & 1) {
                        tcg_gen_st_i32(tcg_const_i32(0 /*float32_zero*/), cpu_env, offsetof(CPUState, xmm_regs[reg].XMM_L(1)));
                    }
                    if ((val >> 2) & 1) {
                        tcg_gen_st_i32(tcg_const_i32(0 /*float32_zero*/), cpu_env, offsetof(CPUState, xmm_regs[reg].XMM_L(2)));
                    }
                    if ((val >> 3) & 1) {
                        tcg_gen_st_i32(tcg_const_i32(0 /*float32_zero*/), cpu_env, offsetof(CPUState, xmm_regs[reg].XMM_L(3)));
                    }
                    break;
                case 0x22:
...
do_lcall:
            if (s->pe && !s->vm86) {
                ...
                gen_jmp_im(pc_start - s->cs_base);
                tcg_gen_trunc_tl_i32(cpu_tmp2_i32, cpu_T[0]);
                gen_helper_lcall_protected(cpu_tmp2_i32, cpu_T[1], tcg_const_i32(dflag), tcg_const_i32(s->base.pc - pc_start));
            } else {
                tcg_gen_trunc_tl_i32(cpu_tmp2_i32, cpu_T[0]);
                gen_helper_lcall_real(cpu_tmp2_i32, cpu_T[1], tcg_const_i32(dflag), tcg_const_i32(s->base.pc - s->cs_base));
            }
            gen_eob(s);
            break;
        case 4: /* jmp Ev */
            ...
            if (s->cpl == 0) {
                ...
                if (s->cpl <= s->iopl) {
                    if (s->dflag) {
                        ...
                        gen_helper_write_eflags(cpu_T[0], tcg_const_i32(
                                                    (TF_MASK | AC_MASK | ID_MASK | NT_MASK | IF_MASK) & 0xffff));
                    }
                } else {
                    if (s->dflag) {
...
bt_op:
    ...
    case 0x1bd: /* bsr */
    {
        ...
        if ((b & 1) && (prefixes & PREFIX_REPZ) && (s->cpuid_ext3_features & CPUID_EXT3_ABM)) {
            switch (ot) {
            case OT_WORD: gen_helper_lzcnt(cpu_T[0], t0, tcg_const_i32(16)); break;
            case OT_LONG: gen_helper_lzcnt(cpu_T[0], t0, tcg_const_i32(32)); break;
            case OT_QUAD: gen_helper_lzcnt(cpu_T[0], t0, tcg_const_i32(64)); break;
            }
            gen_op_mov_reg_T0(ot, reg);
        } else {
            label1 = gen_new_label();
