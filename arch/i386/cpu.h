...
static inline void cpu_x86_load_seg_cache(CPUState *env, int seg_reg, unsigned int selector, target_ulong base,
                                          unsigned int limit, unsigned int flags)
{
    ...
    /* update the hidden flags */
    {
        ...
        } else if (!(env->cr[0] & CR0_PE_MASK) || (env->eflags & VM_MASK) || !(env->hflags & HF_CS32_MASK)) {
...
/* helper.c */
int cpu_handle_mmu_fault(CPUState *env, target_ulong addr, int is_write, int mmu_idx, int no_page_fault);
...
static inline void cpu_get_tb_cpu_state(CPUState *env, target_ulong *pc, target_ulong *cs_base, int *flags)
{
    ...
    *pc = *cs_base + env->eip;
    *flags = env->hflags | (env->eflags & (IOPL_MASK | TF_MASK | RF_MASK | VM_MASK));
}
...
void TLIB_NORETURN raise_exception_err_env(CPUState *nenv, int exception_index, int error_code);
