...
typedef struct pmu_counter {
    ...
    // Note that snapshots can only be taken when the counter is capable of counting events (both PMU and the counter are enabled)
    // If the counter changes state in any way (is disabled or changes event) the value MUST be calculated and snapshot reset
    bool has_snapshot;
