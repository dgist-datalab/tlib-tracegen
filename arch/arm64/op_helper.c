                        ...
                        uint32_t target_el, uintptr_t ra)
{
     ...
     */
    cpu_restore_state_and_restore_instructions_count(cs, cs->current_tb, ra, true);
...
void HELPER(v8m_stackcheck)(CPUARMState *env, uint32_t newvalue)
{
    ...
    if (newvalue < v7m_sp_limit(env)) {
         ...
         */
        raise_exception_ra(env, EXCP_STKOF, 0, 1, ARM_GETPC());
    }
}
...
void HELPER(check_bxj_trap)(CPUARMState *env, uint32_t rm)
{
    ...
    if (env->cp15.hstr_el2 & HSTR_TJDBX) {
        ...
        raise_exception_ra(env, EXCP_HYP_TRAP, syn, 2, ARM_GETPC());
    }
}
