...
static inline void gen_bx_excret_final_code(DisasContext *s)
{
    ...
    int excret_label = gen_new_label();
    uint32_t min_magic;
...
MemOp pow2_align(unsigned i)
{
    ...
    tlib_assert(i < ARRAY_SIZE(mop_align));
    return mop_align[i];
}
...
// TODO: Remove the argument after porting 'tcg_gen_lookup_and_goto_ptr'.
static void gen_goto_ptr(DisasContext *dc)
{
                                     ...
                                     int *tgtmode, int *regno)
{
    ...
    case ARM_CPU_MODE_HYP:
        /*
         * r13_hyp can only be accessed from Monitor mode (and so we can
         * forbid accesses from EL2 or below). elr_hyp and SPSR_Hyp can be
         * accessed also from Hyp mode, so forbid accesses from EL0 or EL1.
         */
        if (!arm_dc_feature(s, ARM_FEATURE_EL2) || s->current_el < 2 ||
            (s->current_el < 3 && *regno != 17 && *regno != 16)) {
            goto undef;
        }
        break;
    default:
                           ...
                           bool isread, int rt, int rt2)
{
    ...
    if (ri) {
        ...
        if (s->hstr_active || ri->accessfn ||
            ...
            default:
                 ...
                 * guest visible.
                 */
                tlib_assert(!arm_dc_feature(s, ARM_FEATURE_V8));
     ...
     */
    if (is64) {
        tlib_printf(LOG_LEVEL_WARNING, "%s access to unsupported AArch32 "
...
static bool op_div(DisasContext *s, arg_rrr *a, bool u)
{
    ...
    t1 = load_reg(s, a->rn);
    t2 = load_reg(s, a->rm);
    // TODO: Find out what 'cpu_env' should be used for.
    if (u) {
...
{
    tlib_assert(!arm_dc_feature(s, ARM_FEATURE_THUMB2));
...
static bool trans_WLS(DisasContext *s, arg_WLS *a)
{
    ...
    if (a->size != 4) {
         ...
         */
        tlib_assert(vfp_access_check(s));
        store_cpu_field(tcg_constant_i32(a->size), v7m.ltpsize);
        /*
...
void arm_tr_init_disas_context(DisasContextBase *dcbase, CPUState *env)
{
    ...
    dc->page_start = dc->base.tb->pc & TARGET_PAGE_MASK;
    ...
    // if (!dc->thumb) {
    //     int bound = -(dc->base.tb->pc | TARGET_PAGE_MASK) / 4;
...
static bool arm_check_ss_active(DisasContext *dc)
{
    if (dc->ss_active && !dc->pstate_ss) {
         ...
         * bits should be zero.
         */
        tlib_assert(get_dcbase_num_insns(dc->base) == 1);
...
void thumb_tr_translate_insn(DisasContextBase *dcbase, CPUState *env)
{
    ...
    tlib_assert((dc->base.pc_next & 1) == 0);
...
void arm_tr_tb_stop(DisasContextBase *dcbase, CPUState *cpu)
{
    ...
    if (dc->base.is_jmp == DISAS_BX_EXCRET) {
        ...
        case DISAS_SWI:
            gen_ss_advance(dc);
            gen_exception(EXCP_SWI_SVC, syn_aa32_svc(dc->svc_imm, dc->thumb));
            break;
        case DISAS_HVC:
            gen_ss_advance(dc);
...
// TODO: uncomment when conflicting declaration for gen_intermediate_code will be fixed
...
// TODO: conflicting declaration with exec-all.h
